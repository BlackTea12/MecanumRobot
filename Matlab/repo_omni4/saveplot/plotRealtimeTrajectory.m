close all; clc; cla; 
filename = 'track1124.gif';
for i = 1:length(robotpathObj.time)
    if i == 1
        ...
        init_robot_state = plot(robotpathObj.state(1,i),robotpathObj.state(2,i),'g.');
        p_reftr = plot(robotpathObj.refstate(1,i),robotpathObj.refstate(2,i),'bo');
        ...
        subplot(1,2,2);
        plot(robotpathObj.time(i), robotpathObj.wheelvel(1,i),'r.','MarkerSize',5); hold on; grid on;
        plot(robotpathObj.time(i), robotpathObj.wheelvel(2,i),'g.','MarkerSize',5);
        plot(robotpathObj.time(i), robotpathObj.wheelvel(3,i),'b.','MarkerSize',5);
        plot(robotpathObj.time(i), robotpathObj.wheelvel(4,i),'k.','MarkerSize',5);
        ...
        xlim([0 50]); ylim([-60 60]);
        sgtitle('Performance of Robot','fontsize',17, 'fontweight','bold');
        
    else
        subplot(1,2,1);        
        traj = plot(robotpathObj.state(1,i),robotpathObj.state(2,i),'g.');
        traj.Annotation.LegendInformation.IconDisplayStyle = 'off';    
        p_reftr = plot(robotpathObj.refstate(1,i),robotpathObj.refstate(2,i),'bo');
        p_reftr.Annotation.LegendInformation.IconDisplayStyle = 'off';        
        
        subplot(1,2,2);
        w_1 = plot(robotpathObj.time(i), robotpathObj.wheelvel(1,i),'r.','MarkerSize',5);
        w_2 = plot(robotpathObj.time(i), robotpathObj.wheelvel(2,i),'g.','MarkerSize',5);
        w_3 = plot(robotpathObj.time(i), robotpathObj.wheelvel(3,i),'b.','MarkerSize',5);
        w_4 = plot(robotpathObj.time(i), robotpathObj.wheelvel(4,i),'k.','MarkerSize',5);
